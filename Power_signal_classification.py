...
def drawing_confusion_matrix(uni_uniq_kernel, num_class, y_pred, y_true):
    ...
    df_cm = pd.DataFrame(ratio_matrix, index = [i for i in te_uniq_kernel],
    ...
    output_file=result_dir / "confusion_matrix"
...
if __name__ == '__main__':
    ...
    assert test_df.shape[1] == train_df.shape[1], "test and train data should have same number of samples"
    ...
    train_dataloader = DataLoader(dataset=MyDataset(train_df), batch_size=10, shuffle=False)
    test_dataloader  = DataLoader(dataset=MyDataset(test_df), batch_size=10, shuffle=True)

...
def draw_multigraph(input,output_file):
  ...
  print(fm.findfont("Calibri")) # Test with "Special Elite" too


  print(matplotlib.get_configdir())
   
  sns.set_theme('paper', 'whitegrid', 'Blues', font_scale=1)
  ...
  g= sns.relplot(
    data=input, kind="line", x ="Timestamp",
    y="Power", hue="Freq_mode", 
    # style="Freq_mode", 
    palette=cp,
    col="Kernel", col_wrap=6,
    height=3, aspect=.75, linewidth=2,
  ...
  plt.savefig(str(output_file)+".pdf", format='pdf',bbox_inches='tight',dpi=600)
  return g
  

  # Define the font properties you want to use
  title_properties = {'family': 'Calibri', 'size': 13, 'weight': 'bold', 'color': 'blue'}

  x_label = g.ax.get_xlabel()
  y_label = g.ax.get_ylabel()

  legend = g._legend  # Use the '_legend' attribute to access the legend
...
# Graph
  ...
  for row, ax_row in enumerate(axes):
    for column, ax_column in enumerate(ax_row):
      ...
      sns.lineplot(data=org_df[workload],ax=ax_column,label="Baseline",color=baseline_color,linewidth=1.0)
      sns.lineplot(data=dvfs_df[workload],ax=ax_column,label="HertzPatch",color=Hertzpatch_color,linewidth=1.0)
  ...
  fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(-1.5,1.05),ncol=2,fontsize=12)  
...
def multi_graph(df):
  # g = sns.FacetGrid(df, col="subject", col_wrap=4, height=2, ylim=(0, 10))
  g = sns.FacetGrid(df, col="subject", col_wrap=4, height=2)
...
def collect_data(benchmark, freq_mode, interval):
  ...
  for file in glob.iglob(f"./**/output_{freq_mode}_*_{interval}ms.csv", recursive=True):
...
def collect_data_freq_mode(benchmark, freq_mode_list, interval):
  ...
  filt_wide_df = filt_acc_df.pivot(index=["Kernel","Freq_mode"], columns="Timestamp", values="Power")
...
if __name__ == '__main__':
  ...
  p = sns.barplot(data = Perf_df ,x=Perf_df['Workload'],y=Perf_df['Perf'],
                  edgecolor = "#192133",linewidth=2.0,color=color)
